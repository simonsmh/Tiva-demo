...
/* Peripheral definitions for DY-Tiva board */
// SSI port
...
void xmit_spi(BYTE dat)
{
    ...
    while(ROM_SSIBusy(SDC_SSI_BASE))
        ;
#endif
...
void power_on (void)
{
    ...
    ROM_GPIOPadConfigSet(SDC_GPIO_PORT_BASE, SDC_SSI_FSS,
                         GPIO_STRENGTH_8MA, GPIO_PIN_TYPE_STD_WPD);
                         
    /* Configure the SSI0 port */
    ROM_SSIConfigSetExpClk(SDC_SSI_BASE, ROM_SysCtlClockGet(),
                           SSI_FRF_MOTO_MODE_3, SSI_MODE_MASTER, 100000, 8);
    // sdcard is able to work on mode 0(better)/3
