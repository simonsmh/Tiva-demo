...
float    SP_INT = 25.0 ;   //内部设定值 “内部设定值”输入端用于指定一个设定值。
...
unsigned char    MAN_ON = FALSE;   //手动操作打开 如果“手动操作打开”输入被置位，则中断控制回路。MAN 手动值被设置为可调节变量的值。
//END_VAR
//VAR
...
float    TUN_DLMN = 40.0 ;  //用于过程激励的可调节变量增量
...
//float    PFAC_SP = 0.8 ;   //用于设定值变化的比例因子
////当有设定值变化时，PFAC_SP 指定有效的比例P 作用。它的设置范围是0 到1。
////. 1：如果设定值发生变化，则比例P 作用具有全部效果。
////. 0：如果设定值发生变化，比例P 作用没有任何效果。
//float    GAIN = 191.5511 ;   //比例增益
////“比例增益”输入用于指定控制器增益。可以通过给GAIN 一个负号来反转控制方向。
//float    TI = 6 ;  //复位时间[秒]
////“复位时间”输入(积分时间)决定了积分作用响应。
//float    TD = 1.5 ;  //微分时间[秒]
...
//float    D_F = 5.0 ;   //微分因子
////微分因子描述了D 作用的滞后。D_F = 微分时间/“D 作用的滞后”
//float    CON_ZONE = 1.305135 ; //控制带
////如果偏差大于控制带宽度，将输出可调节变量上限作为实际的可调节变量。
////如果偏差小于控制带宽度，将输出可调节变量下限作为实际的可调节变量。
//unsigned char    CONZ_ON = TRUE ; //控制带打开	CONZ_ON =TRUE 激活控制带。
...
static inline long TIME_TCK(void)
{
  return (Time_zzd);
}
